...
from transformers import AutoModelForCausalLM, FOGConfig, FOGForCausalLM, GenerationConfig
...
def add_arguments(parser):
    group = parser.add_argument_group(title="FOG HF saver.")
    group.add_argument(
        "--hf-tokenizer",
        type=str,
        default=None,
...
def save_checkpoint(queue: mp.Queue, args):
    ...
    if md.checkpoint_args.position_embedding_type != "rope":
        raise ValueError("FOG model must use RoPE")
    if md.checkpoint_args.use_rope_scaling:
        raise ValueError("FOG model must use llama3 RoPE scaling")
    if md.checkpoint_args.normalization != "RMSNorm":
        raise ValueError("FOG model must not use mlp bias")
    if md.checkpoint_args.add_bias_linear:
        raise ValueError("FOG model must use RMSNorm")
    if md.checkpoint_args.swiglu:
        ...
        print("Defaulting to GeLU because none of swiglu, xielu, smoothswiglu was passed")
        activation = "gelu"
    ...
    with TemporaryDirectory(prefix=str(save_dir/"tmp")) as tmp_save_dir:
        ...
        llama_conf = FOGConfig(
            ...
            layerscale=md.checkpoint_args.layer_scale is not None,
            final_layernorm=md.checkpoint_args.final_layernorm,
            softmax_scale=md.checkpoint_args.softmax_scale,
        )
        ...
        for i_layer in range(llama_conf.num_hidden_layers):
            ...
            if activation == "swiglu" or activation == "smoothswiglu":
                state_dict |= {
                ...
                state_dict |= {f"model.layers.{i_layer}.mlp.up_proj.weight": message["mlp l0 weight"]}
            ...
            if md.checkpoint_args.qk_layernorm:
                if llama_conf.qk_type == "rms":
                    ...
                    state_dict |= {
                        f"model.layers.{i_layer}.self_attn.q_norm.weight": message["q norm weight"],
                        f"model.layers.{i_layer}.self_attn.k_norm.weight": message["k norm weight"],
                        f"model.layers.{i_layer}.self_attn.q_norm.alpha": message["q dyt alpha"],  # not there (will be inited by HF during conversion)
                        f"model.layers.{i_layer}.self_attn.k_norm.alpha": message["k dyt alpha"],  # not there  (same)
                    }
        ...
        if llama_conf.final_layernorm==True and md.checkpoint_args.untie_embeddings_and_output_weights==False:
            print("--- Tied embeddings and lm-head ---")
            finalln=queue_get("final norm")["weight"]
            # _ = queue_get("output layer")["weight"]  # no output layer will be sent
            state_dict = {
                "model.norm.weight": finalln,
                "lm_head.weight": embeddings
                }
        elif llama_conf.final_layernorm==True and md.checkpoint_args.untie_embeddings_and_output_weights==True: 
            print("--- finalLN and untied embeddings ---")  
            state_dict = {
                "model.norm.weight": queue_get("final norm")["weight"],
                "lm_head.weight": pad_weight(queue_get("output layer")["weight"], md.true_vocab_size) 
                }
        elif llama_conf.final_layernorm==False and md.checkpoint_args.untie_embeddings_and_output_weights==True:  
            print("--- no finalLN + untied embeddings ---") 
            _ = queue_get("final norm") # is None, but has to be received
            lmhead=queue_get("output layer")["weight"]
            state_dict = {"lm_head.weight": pad_weight(lmhead, md.true_vocab_size) 
                    }
        else:
            print("--- No final LN + tied embeddings ---")
            _ = queue_get("final norm")         # is None, but has to be received
            # _ = queue_get("output layer")     # no output layer will be sent
            state_dict = {
                "lm_head.weight": embeddings
            }
        ...
        model = FOGForCausalLM.from_pretrained(
